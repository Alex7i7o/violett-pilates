...
if "const [showConfirmPlantilla" not in c:
    c = c.replace(state_marker, state_marker + "\n" + state_injection)

# 2. Add handleAssignPlantilla function definition
func_marker = "const handleAssign = async (turnoId: string) => {"
func_injection = """
...
"""
if "const handleAssignPlantilla" not in c:
...
"""
if "Confirmar Horario Fijo" not in c:
