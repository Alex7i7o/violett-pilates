...
export function Dashboard() {
  ...
  const handleCancelRecurrencia = async (id: string) => {
    try {
      ...
      refetchProfile()
      refetchTurnos()
      toast.success("Horario fijo cancelado eééxitosamente.");
    } catch (err: any) {
      ...
      {(profileLoading || bookingsLoading) ? (
        ...
        >
          ...
          <div className="w-full max-w-5xl mx-auto space-y-8 p-4">
            <div className="h-32 bg-primary-light rounded-2xl animate-pulse"></div>
            <div className="h-64 bg-primary-light/50 rounded-2xl animate-pulse"></div>
            <div className="h-64 bg-primary-light/50 rounded-2xl animate-pulse"></div>
          </div>
        </motion.div>
      ) : !profile ? null : (
