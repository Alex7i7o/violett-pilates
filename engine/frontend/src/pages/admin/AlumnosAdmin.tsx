...
export function AlumnosAdmin() {
  ...
  const labelPlural = isEstética ? 'pacientes' : 'alumnas';
  const labelSingular = isEstética ? 'paciente' : 'alumna';
  const LabelPluralTitle = isEstética ? 'Pacientes' : 'Alumnas';
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                ...
                {showFilters && (
                  ...
                  >
                    <div className="pt-4 mt-4 border-t border-primary-light grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <InputField label="Edad Mínima" name="minAge" type="number" value={minAge} onChange={e => setMinAge(e.target.value)} />
      ...
      <ConfirmModal
        ...
        message={`¿Seguro que deseas eliminar a est@ ${labelSingular}? Esta acción no se puede deshacer.`}
