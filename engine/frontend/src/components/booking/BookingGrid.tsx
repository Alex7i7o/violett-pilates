...
export function BookingGrid({
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        {[1, 2, 3].map(i => (
          <div className="bg-gray-200 animate-pulse rounded-xl"  key={i} className="h-32 w-full" />
        ))}
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-semibold text-primary-main">Agenda Disponible</h2>
      ...
      {availableDates.length > 0 && (
        <div className="flex overflow-x-auto gap-2 pb-2 scrollbar-hide snap-x">
          {availableDates.map(date => {
            ...
            const availableSpots = dayTurnos.reduce((acc, t) => acc + t.availableSpots, 0)
            ...
            return (
              <button
                ...
                className={`snap-start relative flex flex-col items-center justify-center min-w-[100px] px-4 py-3 rounded-2xl transition-colors duration-200 z-10 ${
                  isActive 
                    ? 'text-white' 
                    : 'bg-white text-muted hover:bg-primary-light/50'
                }`}
              >
                {isActive && (
                  <motion.div 
                    layoutId="activeDateTabPill"
                    className="absolute inset-0 bg-primary-main rounded-2xl shadow-lg"
                    style={{ zIndex: -1 }}
                ...
                <span className={`text-xs mt-1 px-2 py-0.5 rounded-full transition-colors duration-200 ${
                  isActive ? 'bg-white/20 text-white' : availableSpots > 0 ? 'bg-primary-light text-primary-main' : 'bg-red-100 text-red-900'
                }`}>
          ...
          {displayedTurnos.length > 0 ? (
            ...
            >
              ...
              {displayedTurnos.map((turno) => (
                ...
                <Card className={`transition-all hover:shadow-md ${turno.isBookedByMe ? "border-violett-400 bg-primary-light/50/30" : ""}`}>
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle className="text-xl">{turno.time}</CardTitle>
                        <p className="text-sm font-medium text-primary-main mt-1">{turno.classType}</p>
                      </div>
                      ...
                      {!turno.isBookedByMe && turno.availableSpots > 0 && (
                        turno.allowsRecurring 
                          ? <Badge variant="secondary" className="bg-primary-light text-primary-hover">Clase fija</Badge> 
