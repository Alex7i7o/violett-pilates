    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-2xl shadow-violett-900/5">
        <CardContent className="p-8">
          <div className="flex justify-center mb-8">
            <div className="w-12 h-12 rounded-2xl bg-primary-main flex items-center justify-center">
          ...
          {status === 'success' ? (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
              ...
              <button onClick={() => navigate('/login')} className="text-primary-hover font-bold underline">Volver al inicio de sesión</button>
            </motion.div>
          ) : (
            <>
      ...
      onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="mt-6 text-center">
                <button onClick={() => navigate('/login')} className="text-sm text-slate-500 hover:text-primary-hover underline">Volver atrás</button>
              </div>
            </>
          )}
        </CardContent>
