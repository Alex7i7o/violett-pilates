            ...
            {localData.turnos_hoy.map((t) => (
              <Card key={t.id} className="relative overflow-hidden transition-all duration-300 hover:border-violett-300">
                <CardContent className="p-0">
                  {/* Header clickeable */}
                  <div 
                    className="p-4 cursor-pointer flex justify-between items-center bg-white hover:bg-primary-light/50/50 transition-colors"
                  ...
                  <AnimatePresence>
                    {expandedIds.includes(t.id) && (
                      ...
                      >
                        <div className="p-4 pt-0 border-t border-primary-light bg-primary-light/50/20">
                          <h4 className="text-sm font-bold text-primary-main mb-3 mt-4">Asistencia de la Clase</h4>
                          {(!t.reservas_list || t.reservas_list.length === 0) ? (
                            ...
                            <div className="space-y-3">
                              {t.reservas_list.map((res: any) => (
                                <div key={res.id} className="flex flex-col sm:flex-row justify-between sm:items-center p-3 bg-white border border-primary-light shadow-sm rounded-xl transition-all">
                                  <div>
                                        ...
                                        {(res.estado === 'CONFIRMADA' || res.estado === 'TOMADA') && (
                                          ...
                                          >
                                            <FeedbackButton 
                                              ...
                                              className={res.estado !== 'TOMADA' ? "w-full flex-1 whitespace-nowrap bg-white hover:bg-primary-light/50 text-primary-main border-primary-light hover:border-violett-300" : "w-full flex-1 whitespace-nowrap bg-primary-main hover:bg-primary-hover border-violett-900 text-white"}
