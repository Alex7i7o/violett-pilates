...
ssh root@$SERVER_IP "cd /root/ViolettPilates && unzip -o /root/ViolettPilates-Deploy.zip -d /root/ViolettPilates && docker compose build --no-cache frontend_pilates frontend_estetica && docker compose up -d --build && docker compose exec backend_pilates python manage.py makemigrations && docker compose exec backend_pilates python manage.py migrate && docker compose exec backend_estetica python manage.py migrate && docker compose restart nginx_proxy"
